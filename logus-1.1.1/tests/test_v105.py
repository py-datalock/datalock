...
Suite de testes para as features v1.0.5:
  ...
  - lg.read() big-data params
  - DatabaseConnection / lg.db()
  - lg.validate() (v1.1)
  - lg.lineage (v1.1)
  - lg.mask_sql() (v1.1)
  - privacy_score (v1.1)
...
class TestMaskLazyFrame:
    def test_returns_lazyframe(self, df_pii):
        lf = df_pii.lazy()
        result = lg.mask(lf, salt=SALT)
        assert isinstance(result, pl.LazyFrame)

    def test_masks_pii_columns(self, df_pii):
        lf = df_pii.lazy()
        result = lg.mask(lf, salt=SALT).collect()
    ...
    def test_pipeline_stays_lazy(self, df_pii):
        ...
        masked = lg.mask(pipeline, salt=SALT)
    ...
    def test_columns_filter(self, df_pii):
        lf = df_pii.lazy()
        result = lg.mask(lf, salt=SALT, columns=["cpf"]).collect()
...
class TestStreamCsvBatched:
    ...
    def test_stream_progress_callback(self, csv_file):
        ...
        def on_progress(n, done, total):
            calls.append((n, done))
        list(lg.stream(str(csv_file), chunksize=20, on_progress=on_progress))
        assert len(calls) > 0
...
# ---------------------------------------------------------------------------
# 5. lg.read() big-data params
# ---------------------------------------------------------------------------

class TestReadBigData:
    def test_header_only_parquet(self, parquet_file):
        result = lg.read(str(parquet_file), header_only=True)
    ...
    def test_header_only_csv(self, csv_file):
        result = lg.read(str(csv_file), header_only=True)
        assert isinstance(result, dict)
        assert "columns" in result
        assert "cpf" in result["columns"]

    def test_head_parquet(self, parquet_file):
        result = lg.read(str(parquet_file), head=10)
        assert isinstance(result, pl.DataFrame)
        assert len(result) == 10

    def test_head_csv(self, csv_file):
        result = lg.read(str(csv_file), head=5)
        assert isinstance(result, pl.DataFrame)
        assert len(result) == 5

    def test_tail_parquet(self, parquet_file, df_pii):
        result = lg.read(str(parquet_file), tail=10)
        assert isinstance(result, pl.DataFrame)
        assert len(result) == 10

    def test_sample_parquet(self, parquet_file):
        result = lg.read(str(parquet_file), sample=20)
        assert isinstance(result, pl.DataFrame)
    ...
    def test_sample_csv_warns(self, csv_file):
        with warnings.catch_warnings(record=True) as w:
            ...
            result = lg.read(str(csv_file), sample=10)
    ...
    def test_n_chunks_chunks_parquet(self, parquet_file):
        ...
        result = lg.read(str(parquet_file), n_chunks=3, chunks=[1])
        assert isinstance(result, pl.DataFrame)
        assert len(result) > 0
    ...
    def test_columns_parquet(self, parquet_file):
        result = lg.read(str(parquet_file), columns=["cpf", "renda"])
        assert isinstance(result, pl.DataFrame)
        assert list(result.columns) == ["cpf", "renda"]

    def test_sample_with_salt(self, parquet_file):
        result = lg.read(str(parquet_file), sample=20, salt=SALT)
        assert isinstance(result, pl.DataFrame)
...
class TestCsvIndex:
    ...
    def test_second_read_uses_sidecar(self, csv_file):
        ...
        t = time.perf_counter()
        r1 = lg.read(str(csv_file), n_chunks=3, chunks=[1])
        t1 = (time.perf_counter() - t) * 1000

        t = time.perf_counter()
        r2 = lg.read(str(csv_file), n_chunks=3, chunks=[1])
...
# ---------------------------------------------------------------------------
# 7. DatabaseConnection / lg.db()  (no real DB required — tests interface only)
# ---------------------------------------------------------------------------

class TestDatabaseConnection:
    def test_db_factory_returns_connection(self):
        banco = lg.db("sqlite:///test.db", salt=SALT)
        assert isinstance(banco, lg.DatabaseConnection)

    def test_repr_hides_credentials(self):
        banco = lg.db("postgresql://secret_user:secret_pass@host/db")
        repr_str = repr(banco)
    ...
    def test_read_with_salt_masks(self, tmp_path):
        """lg.read(banco, tabela, salt=) deve mascarar automaticamente."""
        ...
        banco = lg.db(f"sqlite:///{db_path}", salt=SALT)
        df = lg.read(banco, "t")
    ...
    def test_lg_write_banco_syntax(self, tmp_path):
        """lg.write(df, banco, 'tabela') deve funcionar."""
    ...
    def test_read_without_salt_no_masking(self, tmp_path):
        """lg.db() sem salt= não deve mascarar."""
        ...
        banco = lg.db(f"sqlite:///{db_path}")  # sem salt
        df = lg.read(banco, "t")
...
class TestValidate:
    ...
    def test_column_not_found(self, df_pii_pd):
        result = lg.validate(df_pii_pd, {"nao_existe": {"not_null": True}})
        assert not result.passed
...
class TestLineage:
    ...
    def test_to_json_serializable(self, df_pii):
        import json
        tracker = lg.lineage.start("json_test")
    ...
    def test_session_context_manager(self, df_pii, capsys):
        with lg.lineage.session("ctx_test") as lin:
    ...
    def test_openlineage_format(self, df_pii):
        tracker = lg.lineage.start("ol_test")
        tracker.origin(df_pii, "test.parquet")
    ...
    def test_export_operation(self, df_pii):
        tracker = lg.lineage.start("export_test")
        tracker.origin(df_pii, "test.csv")
    ...
    def test_salt_fingerprint_not_full_salt(self, df_pii):
        ...
        tracker = lg.lineage.start("salt_test")
...
class TestMaskSql:
    ...
    def test_generate_view(self, df_pii):
        reports = lg.scan(df_pii)
        view = lg.generate_view(df_pii, "clientes", reports=reports,
    ...
    def test_invalid_dialect_raises(self, df_pii):
        ...
        with pytest.raises(ValueError, match="Dialeto"):
            lg.mask_sql("SELECT * FROM t", reports=reports, dialect="invalid_db")
...
class TestPrivacyScore:
    ...
    def test_masked_data_score_is_valid(self, df_pii):
        ...
        df_safe = lg.mask(df_pii, salt=SALT)
        reports_raw    = lg.scan(df_pii)
        reports_masked = lg.scan(df_safe)
...
# ---------------------------------------------------------------------------
# 12. lg.process() (Item 6.2 — v1.2 module)
# ---------------------------------------------------------------------------

class TestProcess:
    def test_process_basic(self, df_pii):
        result = lg.process(df_pii, salt=SALT)
...
class TestBackwardCompat:
    def test_lg_read_csv(self, csv_file):
        df = lg.read(str(csv_file))
        assert isinstance(df, pl.DataFrame)

    def test_lg_read_parquet(self, parquet_file):
        df = lg.read(str(parquet_file))
        assert isinstance(df, pl.DataFrame)

    def test_lg_mask_polars(self, df_pii):
        result = lg.mask(df_pii, salt=SALT)
        assert isinstance(result, pl.DataFrame)
        assert result["cpf"][0] != df_pii["cpf"][0]

    def test_lg_mask_pandas(self, df_pii_pd):
        result = lg.mask(df_pii_pd, salt=SALT)
    ...
    def test_lg_store_read_lgs(self, tmp_path, df_pii):
        p = tmp_path / "test.dlk"
        lg.store(df_pii.to_pandas(), str(p), key=KEY, overwrite=True)
        df_back = lg.read(str(p), key=KEY)
        assert df_back.shape == df_pii.shape

    def test_lg_where(self, df_pii):
        result = lg.where(df_pii, uf="SP")
        assert all(result["uf"] == "SP")

    def test_lg_groupby(self, df_pii):
        result = lg.groupby(df_pii, "uf", {"n": ("*", "count")})
    ...
    def test_lg_diff(self, df_pii):
        df_safe = lg.mask(df_pii, salt=SALT)
        report = lg.diff(df_pii.to_pandas(), df_safe.to_pandas())
    ...
    def test_lg_join_safe(self, df_pii):
        ...
        result = lg.join(df_pii, df_p, on="cpf", salt=SALT)
