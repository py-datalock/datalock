...
logus/processor.py
==================
lg.process() — Pipeline completo em uma chamada.
...
class ProcessResult:
    """
    Resultado completo de lg.process().
    ...
    profile:     Dict           # dict completo do lg.profile()
    ...
    def print_summary(self) -> None:
        ...
        print(f"\n{'='*60}")
        print(f"  logus.process() — Relatório")
        print(f"{'='*60}")
...
def process(
    ...
    **read_kwargs,
    ...
    Executa em sequência:
      1. Leitura (qualquer formato, incluindo .lgs)
    ...
    Args:
        ...
        output:       Caminho de saída (.lgs, .parquet, .csv, etc.).
        ...
        rules:        Regras de validação para lg.validate().
        where:        Filtro pré-mascaramento (mesmo syntax do lg.where()).
        ...
        **read_kwargs: Repassados ao lg.read() (sep=, encoding=, etc.).
    ...
    Exemplos:
        ...
        result = lg.process("clientes.csv", salt=SALT, key=KEY, output="safe.dlk")
        ...
        result = lg.process(df, salt=SALT)
        df_safe = result.df

        # Só diagnosticar (sem mascarar)
        result = lg.process("clientes.parquet")
    ...
    if isinstance(source, (pl.DataFrame, pd.DataFrame)):
        ...
        df = lg.read(source, key=read_key, **read_kwargs)
    ...
    # Lineage — origem
    if track_lineage:
        lineage_tracker = lg.lineage.start(f"process({Path(source_name).name if isinstance(source, str) else 'df'})")
    ...
    if salt or risk:
        ...
        if salt or risk:
            df_safe = lg.mask(
                df,
                salt=salt or lg.generate_salt(),  # salt aleatório se risk= sem salt=
                columns=columns,
                exclude=exclude,
                risk=risk,
                verbose=False,
    ...
    # ── 6. Diagnóstico ────────────────────────────────────────────────────
    try:
        profile_result = lg.profile(df_safe)
    except Exception:
