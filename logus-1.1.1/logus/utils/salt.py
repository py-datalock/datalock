...
utils/salt.py
=============
Geração criptograficamente segura de salts para uso com lg.mask() e lg.read().
...
def generate_salt(length: int = 48) -> str:
    """
    Gera um salt criptograficamente seguro para uso com lg.mask() e lg.read().
    ...
    - Guarde o salt no vault (AWS Secrets Manager, HashiCorp Vault, Azure Key Vault)
      ou em variável de ambiente (LOGUS_SALT), nunca em código-fonte.
