...
def where(df, expr=None, **kw):
    ...
    Sintaxes (sem import polars):
      kwargs      lg.where(df, uf='SP')
                  lg.where(df, uf='SP', tipo_pessoa='PF')
      lista=isin  lg.where(df, uf=['SP','RJ','MG'])
      range       lg.where(df, renda_mensal=(5000, 15000))
      operadores  lg.where(df, renda_mensal=('>', 5000))
                  lg.where(df, uf=('!=', 'SP'))
                  lg.where(df, nome=('contains', 'Silva'))
                  lg.where(df, nome=('startswith', 'Ana'))
                  lg.where(df, nome=('endswith', '.br'))
                  lg.where(df, nome=('like', '%Silva%'))
                  lg.where(df, doc=('isnull',))
                  lg.where(df, doc=('notnull',))
      None=null   lg.where(df, doc=None)
      lg.col()    lg.where(df, lg.col('renda') > 5000)
      pl.Expr     lg.where(df, pl.col('uf') == 'SP')
      string      lg.where(df, 'uf == "SP" and renda > 5000')
      callable    lg.where(df, lambda d: d['uf'] == 'SP')
      LazyFrame   lg.where(lf, pl.col('uf') == 'SP')  → LazyFrame

    Exemplos:
        lg.where(df, uf='SP')
        lg.where(df, uf=['SP','RJ'])
        lg.where(df, renda_mensal=('>', 5000))
        lg.where(df, nome=('contains', 'Silva'))
        lg.where(df, lg.col('renda').is_between(5000, 15000))
        lg.where(df, ~lg.col('inadimplente'))
        lg.where(df, (lg.col('uf')=='SP') & (lg.col('renda')>5000))
    """
    try:
...
def sort(df, by, *, desc: bool=False,
    ...
    Exemplos:
        ...
        lg.sort(df, ['uf','renda'], desc=True)          # multi-coluna
        lg.sort(df, ['uf','renda'], ascending=[True,False])
    """
...
def groupby(df, by, agg: Dict[str,Any], *,
    ...
    Exemplos:
        ...
        lg.groupby(df, 'uf', {'media':('renda_mensal','mean'), 'n':('*','count')})
        lg.groupby(df, ['uf','tipo'], {'renda':['mean','sum']})
        lg.groupby(df, 'uf', {'media':('renda_mensal','mean'), 'n':('*','count')},
...
def fill_null(df, value) -> AnyFrame:
    ...
    Exemplos:
        lg.fill_null(df, 0)
        lg.fill_null(df, {'renda_mensal': 0, 'uf': 'DESCONHECIDO'})
        lg.fill_null(df, 'forward')
    """
    if _ipl(df) or _ilazy(df):
...
def concat(frames: List[AnyFrame], *, axis: int=0) -> AnyFrame:
    ...
    Exemplos:
        lg.concat([df_jan, df_fev, df_mar])         # empilha linhas
...
def melt(df, *, id_cols=None, value_cols=None,
    ...
    Exemplos:
        lg.melt(df, id_cols=['uf','tipo'], value_cols=['renda_mensal','idade'])
    """
    was_pd = isinstance(df, pd.DataFrame) and not _ipl(df)
...
def apply(df, funcs: Dict[str,Any]) -> AnyFrame:
    ...
    Exemplos:
        lg.apply(df, {'uf': str.upper, 'email': str.lower})
        lg.apply(df, {'renda': lambda v: round(v, 2)})
    """
    was_pd = isinstance(df, pd.DataFrame) and not _ipl(df)
...
def cols(df, pattern=None, *, dtype=None, exclude=None) -> List[str]:
    ...
    Exemplos:
        lg.cols(df, 'renda')                         # colunas com 'renda'
        lg.cols(df, ['renda','idade'])               # múltiplos padrões
        lg.cols(df, dtype='String')                  # todas string
        lg.cols(df, dtype='Float64')                 # todas float
        lg.cols(df, dtype='String', exclude=['nome']) # strings exceto nome
        lg.select(df, lg.cols(df, 'renda'))          # combina com select
    """
    sc = schema(df)
    result = list(sc.keys())
...
class _WhenExpr:
    """
    CASE WHEN sem import polars.  Usada em lg.add_column().

    Exemplos:
        lg.when(lg.col('renda') > 10000, 'alta')
          .when(lg.col('renda') > 5000, 'media')
          .otherwise('baixa')
    """
...
def shift(
    ...
    *,
    ...
    Exemplos:
        lg.shift(df, 1)                          # valor do período anterior
        lg.shift(df, -1, columns="renda")        # próximo valor de renda
        lg.shift(df, 3, fill_value=0.0)          # lag 3 com 0 nos nulls
    """
