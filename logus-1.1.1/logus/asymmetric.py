...
Mecanismo (padrão ECIES / RSA-OAEP):
  ...
  4. Armazena a DEK cifrada no header do .lgs

Uso:
    # Gera par de chaves (uma vez por usuário/serviço)
    priv, pub = lg.asymmetric.generate_keypair()
    lg.asymmetric.save_keypair(priv, "minha_chave.pem", pub, "minha_chave_publica.pem")

    # Remetente: cifra com a chave pública do destinatário
    pub = lg.asymmetric.load_public_key("destinatario_pub.pem")
    lg.store(df, "dados.dlk", public_key=pub)

    # Destinatário: decifra com sua chave privada
    priv = lg.asymmetric.load_private_key("minha_chave.pem")
    df   = lg.read("dados.dlk", private_key=priv)

    # Multi-recipient: cifra uma vez para N destinatários
    pubs = [pub_ana, pub_bruno, pub_carlos]
    lg.store(df, "dados.dlk", public_keys=pubs)
    df = lg.read("dados.dlk", private_key=priv_ana)
"""
...
def generate_keypair(
    ...
    Exemplos:
        priv, pub = lg.asymmetric.generate_keypair()
        priv, pub = lg.asymmetric.generate_keypair("rsa", 4096)
    """
...
def encrypt_dek(dek: bytes, public_key: PublicKey) -> bytes:
    ...
    if isinstance(public_key, RSAPublicKey):
        ...
        kek        = HKDF(
            ...
            info=b"logus-lgs-dek-wrap", backend=default_backend()
        ).derive(shared)
        nonce      = os.urandom(12)
