...
logus/contract.py
=================
lg.contract() — Contrato de dados: tipos + PII + mascaramento + validação
...
lg.contract() unifica as três em uma declaração JSON-exportável que pode
...
class FieldSpec:
    ...
    def to_validation_rules(self) -> Dict[str, Any]:
        """Converte para dict de regras compatível com lg.validate()."""
        rules: Dict[str, Any] = {}
...
class DataContract:
    ...
    def mask(self, df: Any, *, salt: str) -> Any:
        ...
        Diferença de lg.mask(): usa as estratégias EXPLÍCITAS do contrato em vez
