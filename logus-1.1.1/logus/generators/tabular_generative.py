...
------------------------------------
  1. lg.mask(df, salt=salt) → df_mascarado
...
def _get_logus_version() -> str:
    """Returns the installed logus version string, with fallback."""
    try:
        import importlib.metadata as _im
        return f"logus/{_im.version('logus')}"
    except Exception:
        try:
            from datalock import __version__  # type: ignore
            return f"logus/{__version__}"
        except Exception:
            return "logus/unknown"
...
class TabularGenerative:
    ...
    Exemplo de uso completo (integrado ao framework):
        ...
        df_mascarado = lg.mask(df_raw, salt=salt)
    ...
    def fit(
        ...
        if len(df) < 100:
            raise ValueError(
                ...
                f"Para datasets pequenos, use lg.mask() em vez de lg.clone()."
            )
            ...
            warnings.warn(
                ...
                f"ou (2) usar lg.mask() para proteção mais robusta com esse volume de dados.",
    ...
    def to_bytes(self) -> bytes:
        ...
        SEGURANÇA: use sempre este método via lg.store() para manter o modelo
    ...
    def load_from_bytes(cls, data: bytes, **kwargs) -> "TabularGenerative":
        ...
        SEGURANÇA: use via lg.read('modelo.dlk', key=...) que mantém os bytes
    ...
    def save(self, path: str) -> None:
        ...
        Para produção, prefira to_bytes() + lg.store() que mantém tudo cifrado.
        """
        import pickle
        if self._model is None:
    ...
    def load(cls, path: str, **kwargs) -> "TabularGenerative":
        """Carrega modelo previamente salvo com .save(). Para modelos em .lgs, use lg.read()."""
        import pickle
        instance = cls(**kwargs)
