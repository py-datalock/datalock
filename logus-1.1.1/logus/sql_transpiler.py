"""
logus/sql_transpiler.py
=======================
SQL Transpiler — lg.mask_sql()
...
def mask_sql(
    ...
    *,
    ...
    Args:
        ...
        salt:        Salt HMAC para hash. None → usa '${LOGUS_SALT}' como placeholder.
    ...
    salt_literal = f"'{salt}'" if salt else "'${LOGUS_SALT}'"
    ...
    if not masked_exprs:
        if annotate:
            return f"-- logus: nenhuma coluna PII detectada no SELECT\n{query}"
        return query
