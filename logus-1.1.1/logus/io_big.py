...
Exporta:
  read_partial()   — motor chamado por lg.read() quando big-data params usados
  build_csv_index() / load_csv_index() — sidecar .csv.logus_idx
  DatabaseConnection — objeto de conexão reutilizável (lg.db())
...
def read_partial(
    ...
    **kwargs,
    ...
    Chamado por lg.read() quando qualquer parâmetro big-data é fornecido:
    ...
    if chunks is not None and n_chunks is None:
        raise ValueError(
            ...
            "Exemplo: lg.read('big.parquet', n_chunks=5, chunks=[2, 4])"
        )
...
def _read_text_format(
    ...
    **kwargs,
    ...
    if sample is not None and is_csv and not iter_chunks:
        warnings.warn(
            f"lg.read('{path.name}', sample={sample}): CSV não tem estrutura de blocos. "
            ...
            f"  lg.write(lg.read('{path.name}', head=1_000_000), '{path.stem}.parquet')\n"
            f"  df = lg.read('{path.stem}.parquet', sample={sample})",
            UserWarning,
            stacklevel=4,
        )
    ...
    if (chunks is not None or iter_chunks) and is_csv:
        ...
        if chunks is not None:
            warnings.warn(
                f"lg.read('{path.name}', n_chunks={n_chunks}, chunks={chunks}): "
...
def build_csv_index(
    ...
    Args:
        ...
        save:     Se True, salva sidecar .csv.logus_idx.
...
class DatabaseConnection:
    ...
    Criado via lg.db(). Integra-se com lg.read() e lg.write().
    ...
    def read(
        ...
        *,
        ...
        if effective_salt:
            ...
            return lg.mask(df, salt=effective_salt, verbose=verbose)
        return df
    ...
    def write(
        ...
        *,
        ...
        if effective_salt:
            import datalock as dd
            df = lg.mask(df, salt=effective_salt)
