...
Por que LGSFile existe além de lg.store() / lg.read()?
--------------------------------------------------------
`lg.store()` e `lg.read()` são convenientes para operações únicas.
...
Padrões suportados:
    ...
    with lg.open("clientes.dlk", key="k") as f:
    ...
    # 2. Fluent API
    df = lg.open("clientes.dlk", key="k").read()
...
class LGSFile:
    ...
    def info(self, *, force: bool = False) -> Dict[str, Any]:
        ...
        if self._info_cache is None or force:
            import datalock as dd
            self._info_cache = lg.inspect(str(self.path), key=self._key)
    ...
    def _require_key(self) -> str:
        if self._key is None:
            raise ValueError(
                ...
                "Passe key= ao criar LGSFile ou use lg.open(..., key='...')."
            )
        return self._key
