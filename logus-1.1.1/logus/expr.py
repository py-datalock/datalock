...
logus/expr.py
=============
lg.col() — expressões de coluna agnósticas ao engine.

Com Polars instalado (obrigatório desde v1.0.4), lg.col() É pl.col() —
...
Este módulo expõe apenas as classes auxiliares que o pipeline interno usa:
  ...
  - ColExpr   → expressão aritmética (lg.col('x') * 0.27)
  - Expr      → expressão de filtro (lg.col('x') > 5)
  - _WhenExpr → CASE WHEN (movida para analytics.py, reexportada aqui por compat)

Nota: desde v1.0.4 lg.col = pl.col diretamente no __init__.py.
...
class Col:
    ...
    Criada via lg.col('nome_da_coluna') quando Polars não está disponível.
    Com Polars instalado, lg.col = pl.col diretamente (mais poderoso).

    Exemplos:
        lg.where(df, lg.col('uf') == 'SP')
        lg.where(df, lg.col('renda').between(5000, 15000))
        lg.where(df, lg.col('uf').is_in(['SP','RJ','MG']))
...
class ColExpr:
    """
    Expressão aritmética. Retornada por lg.col('x') * 0.27 etc.
    Usada em lg.add_column() e convertida para pl.Expr internamente.
    """
    ...
    # Permite encadear: (lg.col('x') * 0.27) + 100
    ...
    def __repr__(self) -> str:
        return f"ColExpr(lg.col({self._col!r}) {self._op} {self._arg!r})"
...
class TransformExpr:
    """Expressão de transformação (não filtro). Usada em lg.add_column()."""
...
class Expr:
    """
    Expressão de filtro lazy — avaliada quando passada para lg.where() etc.

    Suporta &, |, ~ para combinação lógica.

    Exemplos:
        e = lg.col('uf') == 'SP'
        e = lg.col('uf').is_in(['SP','RJ']) & (lg.col('renda') > 5000)
        e = ~lg.col('inadimplente')
    """
    ...
    def __repr__(self) -> str:
        ...
        return f"lg.col({self._col!r}).{self._op}({self._arg!r})"
...
def col(name: str) -> Col:
    ...
    lg.col é diretamente pl.col — com acesso a todos os métodos nativos.
    Esta função é o fallback para uso programático via logus.expr.col().
    """
    return Col(name)
