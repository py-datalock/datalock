...
logus/lineage.py
================
Rastreamento de linhagem de dados — lg.lineage()
...
Uso:
    lg.lineage.track(df, source="clientes.parquet")
    df2 = lg.mask(df, salt=SALT)
    lg.lineage.record_mask(df2, columns=["cpf","email"])
    lg.lineage.export("lineage.json")

    # Ou via context manager
    with lg.lineage.session("pipeline_crm") as lin:
        df = lg.read("clientes.csv")
        lin.origin(df, "clientes.csv", format="csv")
        df2 = lg.where(df, uf="SP")
        lin.transform(df2, operation="filter", detail="uf='SP'")
        df3 = lg.mask(df2, salt=SALT)
        ...
        for node in self.nodes:
            events.append({
                ...
                "run": {
                    ...
                    "facets": {
                        "logus": {
                            "_producer": f"logus/{self.logus_version}",
                ...
                "inputs": [{"namespace": "logus", "name": p} for p in node.parent_ids],
                "outputs": [{
                    "namespace": "logus",
                    "name":      node.node_id,
                    "facets": {
...
class LineageTracker:
    """
    Rastreia a linhagem de dados de um pipeline logus.
