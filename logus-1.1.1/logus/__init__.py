"""
logus — Privacy-by-Design para dados tabulares. LGPD compliance em Python.
...
Uso rápido:
    ...
    lg.store(df_safe, "clientes.dlk", key=KEY)

    # Pipeline fluente
    result = (
        lg.pipe("clientes.parquet")
        .where(uf="SP", tipo_pessoa="PF")
        .add_column(imposto=lg.col("renda_mensal") * 0.27)
        .mask(salt=SALT)
        .collect()
    )
"""
...
# lg.col = pl.col — acesso a todos os métodos nativos Polars (rank, over, str.*, dt.*, etc.)
# lg.when, lg.lit, lg.concat_str também são wrappers Polars nativos
col        = pl.col
lit        = pl.lit
concat_str = pl.concat_str

# lg.when vem de analytics — wrapper que funciona sem import polars
...
__all__ = [
    ...
    "RiskLevel", "ColumnReport", "IdempotencyError", "LGSFile",
    # Sub-namespaces
    "pl", "check", "link",
    # Filtragem
...
def scan(
    ...
    *,
    ...
    Exemplos:
        ...
        reports = lg.scan("clientes.dlk", key=KEY)
    """
...
def mask(
    ...
    *,
    ...
    Exemplos:
        ...
        df_pl_safe = lg.mask(df_pl, salt=SALT)   # retorna pl.DataFrame
        df_safe = lg.mask(df, salt=SALT, risk="high")   # suprime alto risco
    """
    if not salt:
        # Verifica DEFAULT_SALT configurado via lg.configure(default_salt=...)
        import logus._defaults as _defs
        if _defs.DEFAULT_SALT:
            salt = _defs.DEFAULT_SALT
        else:
            import secrets as _sec
            warnings.warn(
                "lg.mask(salt=None): salt aleatório gerado — hashes NÃO serão reprodutíveis "
                "entre execuções. Joins entre tabelas mascaradas em momentos distintos vão quebrar. "
                "Use lg.configure(default_salt=lg.generate_salt()) ou passe salt= explicitamente.",
                UserWarning,
                stacklevel=2,
            )
    ...
    if isinstance(df, pl.LazyFrame):
        ...
        if not _salt:
            import secrets as _sec
            warnings.warn(
                "lg.mask(lf, salt=None): salt aleatório — hashes não reprodutíveis.",
...
def read(
    ...
    **kwargs,
    ...
    Formatos suportados:
        ...
        .dlk                     → logus (AES-256-GCM, multi-frame)
    ...
    Args:
        ...
        raw:       Para .lgs: retorna sem mascaramento adicional.
        frame:     Frame a extrair de .dlk multi-frame.
        chunksize: Lê CSV em chunks (gerador de pd.DataFrame).
        size:      Linhas a gerar de modelo generativo em .lgs.
    ...
    Exemplos:
        ...
        df = lg.read("clientes.dlk", key=KEY)
        df = lg.read("clientes.dlk", key=KEY, salt=SALT)  # descriptografa + mascara
        df = lg.read("clientes.csv", salt=SALT)           # lê + mascara
        df = lg.read("clientes.csv", sep=";", encoding="latin-1")
    """
    # ── DatabaseConnection — objeto de conexão lg.db() ──────────────────
    if isinstance(source, DatabaseConnection):
        if table_or_sql is None:
            raise ValueError(
                "lg.read(banco, ...) requer o nome da tabela ou SQL como segundo argumento. "
                "Exemplo: lg.read(banco, 'clientes')"
            )
        return source.read(
            table_or_sql,
            columns=columns,
    ...
    if raw and ext != ".dlk":
        warnings.warn(
            f"lg.read(raw=True) não tem efeito em arquivos {ext!r}. "
...
def store(
    ...
    *,
    ...
    Exemplos:
        lg.store(df, "dados.dlk", key=KEY)                      # cifrado
        lg.store(df, "dados.dlk", key=KEY, salt=SALT)           # mascara + cifra
        lg.store(df, "dados.dlk", anonymize=True)               # mascara, sem key
        lg.store({"a": df1, "b": df2}, "multi.dlk", key=KEY)   # multi-frame
    """
...
def inspect(path: Union[str, Path], *, key: Optional[str] = None) -> Dict[str, Any]:
    ...
    Exemplos:
        info = lg.inspect("clientes.dlk", key=KEY)
...
def rekey(
    ...
    *,
    ...
    Exemplos:
        lg.rekey("dados.dlk", old_key=KEY_ANTIGO, new_key=KEY_NOVO)
    """
...
def write(
    ...
    **kw,
    ...
    Args:
        ...
        path_or_conn: Caminho de arquivo (str/Path) ou DatabaseConnection (lg.db()).
    ...
    Exemplos:
        ...
        lg.write(df, banco, "clientes_masked")   # banco = lg.db("...")
    """
    if isinstance(path_or_conn, DatabaseConnection):
        if table is None:
            raise ValueError(
                "lg.write(df, banco, ...) requer o nome da tabela como terceiro argumento. "
                "Exemplo: lg.write(df, banco, 'clientes_masked')"
            )
...
def sql(
    ...
    **frames,
    ...
    Requer: pip install 'logus-lgpd[sql]'  (duckdb)
    ...
    try:
        ...
        raise ImportError(
            "lg.sql() requer duckdb. Instale com: pip install 'logus-lgpd[sql]'"
        ) from None
...
def scan_text(text: str) -> List[Dict[str, Any]]:
    ...
    Exemplos:
        lg.scan_text("Meu CPF é 111.444.777-35, email: ana@empresa.com")
    """
...
def mask_text(text: str, *, salt: Optional[str] = None, strategy: str = "redact") -> str:
    """
    ...
    Exemplos:
        safe = lg.mask_text("CPF: 111.444.777-35, email: ana@empresa.com")
        # "CPF: REDACTED, email: REDACTED"
    """
    from datalock.detectors.text_detector import TextPIIDetector
    return TextPIIDetector().mask(text, salt=salt, strategy=strategy)
...
def profile(
    ...
    *,
    ...
    Args:
        source:      DataFrame ou caminho de arquivo.
        key:         Chave de decriptação para .lgs.
    ...
    if not left_masked or not right_masked:
        if salt is None:
            raise ValueError(
                ...
                "Forneça salt= para que logus aplique o mesmo mascaramento em ambos."
            )
...
def configure(
    *,
    ...
    """
    Configura parâmetros globais do logus.
    ...
    Args:
        ...
        load_dotenv:  Se True, carrega variáveis de um arquivo .env (requer python-dotenv).
                      Após carregar, lg.configure lê LOGUS_SALT e LOGUS_KEY automaticamente.
        dotenv_path:  Caminho para o .env. None = procura ".env" no diretório atual.

    Exemplos:
        # Carrega .env automaticamente
        lg.configure(load_dotenv=True)
        # Após isso: LOGUS_SALT e LOGUS_KEY disponíveis via os.environ

        from datalock.reports.audit_report import AuditReport
        lg.configure(audit=AuditReport())
        lg.configure(audit_path="./audit/")
    ...
    if load_dotenv:
        try:
            ...
            # Read LOGUS_SALT and LOGUS_KEY from env if not already set
            import os as _os
            if default_salt is None:
                _env_salt = _os.environ.get("LOGUS_SALT")
                if _env_salt:
                    default_salt = _env_salt
        except ImportError:
            warnings.warn(
                "lg.configure(load_dotenv=True) requer python-dotenv. "
...
def db(
    ...
    *,
    ...
    Integra-se com lg.read() e lg.write() para leitura/escrita unificada.
    ...
    Args:
        ...
        salt:            Salt HMAC. Se fornecido, mascara automaticamente em
                         todo lg.read(banco, ...). Omita para exploração sem mascaramento.
    ...
    Returns:
        DatabaseConnection — passível a lg.read(), lg.write() e context manager.

    Exemplos:
        # Exploração sem mascaramento
        banco = lg.db("postgresql://user:pass@host/db")
        print(banco.tables())
        ...
        lg.write(banco, df_safe, "clientes_masked")
...
def read_db(
    ...
    **kwargs,
    ...
    Exemplos:
        df = lg.read_db("postgresql://u:p@h/db", "clientes", salt=SALT, table=True)
        df = lg.read_db("postgresql://u:p@h/db",
...
from datalock.dlk import LGSFile
...
def open(
    ...
    *,
    ...
    Args:
        path:     Caminho para o arquivo .lgs.
    ...
    Returns:
        LGSFile com métodos read(), write(), frames(), info(), valid(), etc.

    Exemplos:
        with lg.open("clientes.dlk", key=KEY) as f:
            df = f.read()
            print(f.info())
            f.add_frame("novos", df_novos)

        df = lg.open("clientes.dlk", key=KEY).read()
    """
    return LGSFile(path, key=key, salt=salt, compress=compress)
...
def train(df: pd.DataFrame, *, n: int = 1000, random_state: int = 42, **kwargs) -> Any:
    """
    Treina modelo generativo sintético (requer: pip install 'logus-lgpd[synthetic]').
    Retorna modelo que pode ser armazenado em .dlk via lg.store().
    """
    try:
        ...
        raise ImportError(
            ...
            "Instale com: pip install 'logus-lgpd[synthetic]'"
        ) from None
...
save = store   # lg.save() == lg.store()
load = read    # lg.load() == lg.read()


# ---------------------------------------------------------------------------
# Namespace lg.pl — sub-API Polars expostos via logus
# ---------------------------------------------------------------------------

class _PolarsNamespace:
    """
    lg.pl — acesso ao Polars via namespace logus.

    Permite usar Polars sem importar polars diretamente.

    Exemplos:
        df = lg.pl.read_parquet("dados.parquet")
        df = lg.pl.from_pandas(df_pd)
        schema = lg.pl.Schema({"nome": lg.pl.String, "idade": lg.pl.Int32})
    """
    ...
    def __repr__(self) -> str:
        return f"logus.pl (Polars {pl.__version__})"
...
def __getattr__(name: str):
    """Resolve lg.pl, lg.check, lg.link via __getattr__."""
    if name == "pl":
        return pl_ns
    if name == "check":
        return check
    ...
    raise AttributeError(f"módulo 'logus' não tem atributo '{name!r}'")
...
class _Pipeline:
    ...
    Criado via lg.pipe(source) ou lg.pipe().
    ...
    def _ensure(self):
        if self._df is None:
            raise RuntimeError(
                "Pipeline sem dados. Use lg.pipe('arquivo.parquet'), "
...
def _read_chunked(path: str, *, salt, chunksize, verbose, **kwargs):
    """Gerador de chunks para CSV (quando chunksize= passado a lg.read())."""
...
def _validate_key_salt_distinct(key: Optional[str], salt: Optional[str]) -> None:
    if key and salt and key == salt:
        raise ValueError(
            ...
            "Use valores distintos gerados com lg.generate_salt()."
        )
