"""
datalock/validate.py
=================
Data Quality integrado — dd.validate() e dd.expect()
...
Uso:
    ...
    dd.expect(df, "cpf").not_null().unique().matches(r"^\\d{11}$").validate()
"""
    ...
    for col, col_rules in rules.items():
        ...
        for rule_name, rule_val in col_rules.items():
            ...
            if warn_only and not r.passed:
                warnings.warn(
                    f"[datalock.validate] {col}/{rule_name}: {r.detail}",
                    UserWarning,
                    stacklevel=3,
                )
                r.severity = "warning"
...
def save_rules(rules: Dict[str, Dict], path) -> None:
    ...
    Args:
        rules: Dict de regras (mesmo formato aceito por dd.validate()).
        path:  Caminho de saída.

    Exemplos:
        dd.save_rules({"cpf":{"not_null":True},"renda":{"min":0}}, "regras_clientes.json")
    """
    import json
...
def load_rules(path) -> Dict[str, Dict]:
    ...
    Args:
        path: Caminho do arquivo gerado por dd.save_rules().

    Returns:
        Dict de regras prontas para passar a dd.validate().

    Exemplos:
        rules = dd.load_rules("regras_clientes.json")
        result = dd.validate(df, rules)
    """
    import json
    from pathlib import Path
...
def expect(df: Any, column: str) -> _ColumnExpectation:
    ...
    Exemplos:
        dd.expect(df, "cpf").not_null().unique().validate()
        dd.expect(df, "renda_mensal").not_null().between(0, 500_000).validate()
        dd.expect(df, "email").contains("@").min_length(5).validate()
    """
