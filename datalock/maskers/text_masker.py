...
Estratégias disponíveis:
    ...
    "semantic" → substitui por dado sintético válido (ex: CPF → CPF falso válido)
...
class TextMasker:
    ...
    def __init__(
        ...
        if strategy not in ("redact", "hash", "partial", "semantic"):
            raise ValueError(
                f"strategy deve ser 'redact', 'hash', 'partial' ou 'semantic'. Recebido: {strategy!r}"
            )
    ...
    def _replace(self, span: PIISpan) -> str:
        if self._strategy == "redact":
            ...
            return self._semantic_replace(span)
        return "[REDACTED]"

    def _semantic_replace(self, span: PIISpan) -> str:
        """Substitui PII por dado sintético estruturalmente coerente.

        Usa gerador interno (CPF/CNPJ/email matemáticamente válidos) sem
        dependências adicionais. Se faker estiver instalado (pip install faker),
        usa automaticamente para melhor qualidade.

        A substituição é DETERMINÍSTICA: mesmo valor + mesmo salt → mesmo fake.
        Isso preserva joins entre tabelas mascaradas semanticamente.
        """
        from datalock.generators.synthetic import SyntheticGenerator
        gen = SyntheticGenerator(seed=self._hmac_seed(span.text))
        pii_str = span.pii_type.value.lower()
        return gen.generate(pii_str)

    def _hmac_seed(self, value: str) -> int:
        """Deriva semente determinística do valor original (preserva joins)."""
        import struct
        key = (self._salt or "datalock-semantic").encode()
        digest = hmac.new(key, value.encode(), hashlib.sha256).digest()
        return struct.unpack(">I", digest[:4])[0]  # first 4 bytes as uint32
