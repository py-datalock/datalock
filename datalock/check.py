"""
datalock.check — Suite de métricas de privacidade e qualidade.
...
def fidelity(df_real, df_synthetic, *, columns=None, tstr_target=None,
    ...
    Args:
        ...
        df_synthetic: DataFrame sintético gerado pelo dd.clone().
    ...
    Example:
        report = dd.check.fidelity(df_real, df_synth)
        report.print_report()
    """
...
def kanon(df, *, quasi_identifiers, sensitive_attributes=None, target_k=5):
    ...
    Example:
        report = dd.check.kanon(df, quasi_identifiers=["uf", "idade", "escolaridade"])
...
def risk(df, *, quasi_identifiers=None, direct_identifiers=None,
    ...
    Example:
        report = dd.check.risk(df_safe, quasi_identifiers=["uf", "idade"])
...
def utility(df_original, df_masked, *, columns=None):
    ...
    Example:
        report = dd.check.utility(df_original, df_masked)
...
def tcloseness(df, *, quasi_identifiers, sensitive_attribute, target_t=0.2):
    ...
    Example:
        report = dd.check.tcloseness(df, quasi_identifiers=["uf"], sensitive_attribute="diagnostico")
